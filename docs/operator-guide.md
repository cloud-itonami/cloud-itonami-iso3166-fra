...
2. Register the client's intake: business type, target public function,
   prior filing history in France if any.
3. Run the advisor in read-only mode against PLACE (Plateforme des
   Achats de l'État) and BOAMP (Bulletin officiel des annonces des
   marchés publics) for tender-notice discovery.
4. Compare the checklist against the client's current documentation
   (SIREN/SIRET status, electronic-signature certificate).
...
- named referral relationship with French-licensed counsel or a
...
a working referral relationship with French-licensed counsel or a
registered agent for whatever licensed representation the law of
France requires for actual public-procurement filings.
