(ns marketentry.store
  "SSoT for the FRA market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems FR" :portal "PLACE"
             ...
             :requires-siret? true :siret-verified? true
             :drafted? false :submitted? false
             :jurisdiction "FRA" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "PLACE"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-siret? true :siret-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "PLACE"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "FRA" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "PLACE"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "FRA" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "PLACE"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-siret? true :siret-verified? false
             :drafted? false :submitted? false
             :jurisdiction "FRA" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "PLACE"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-siret?)    (assoc :engagement/requires-siret? requires-siret?)
    (some? siret-verified?)    (assoc :engagement/siret-verified? siret-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-siret? :engagement/siret-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-siret? (boolean (:engagement/requires-siret? m))
     :siret-verified? (boolean (:engagement/siret-verified? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-fra"})
